...
# Of length 1 unless just from is supplied as an unnamed 
# argument.
...
# length.out	
# desired length of the sequence. A non-negative number, which 
# for seq and seq.int will be rounded up if fractional.
...
# seq is generic, and only the default method is described here. Note that 
# it dispatches on the class of the first argument irrespective of argument 
# names. This can have unintended consequences if it is called with just one 
# argument intending this to be taken as along.with: it is much better to use 
# seq_along in that case.

# seq.int is an internal generic which dispatches on methods for "seq" based 
# on the class of the first supplied argument (before argument matching).
...
# seq(from)
# seq(length.out= )
# The first form generates the sequence from, 
# from+/-1, ..., to (identical to from:to).
...
# provided length.out or along.with is specified. More generally, the default method 
# of seq will handle classed objects with methods for the Math, Ops and Summary group 
# generics.

# seq.int, seq_along and seq_len are primitive.

# Value
# seq.int and the default method of seq for numeric arguments return a vector 
# of type "integer" or "double": programmers should not rely on which.

# seq_along and seq_len return an integer vector, unless it is a long vector when 
# it will be double.
