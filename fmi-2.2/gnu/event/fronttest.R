...
# For url the description is a complete URL including scheme (such as http://, https://, ftp:// or file://). Method "internal" is that available since connections were introduced.. Method "wininet" is only available on Windows (it uses the WinINet functions of that OS) and method "libcurl" (using the library of that name: https://curl.se/libcurl/) is required on a Unix-alike but optional on Windows. Method "default" uses method "internal" for file:// URLs and uses "libcurl" (if available) for ftp:// and ftps:// URLs. On a Unix-alike it uses "libcurl" for http:// and https:// URLs; on Windows "wininet" for http:// and https:// URLs (and for ftp:// if "libcurl" is unavailable). Which methods support which schemes has varied by R version – currently "internal" supports file://, http:// (deprecated) and ftp:// (deprecated); "wininet" supports file://, http://, https:// and ftp:// (deprecated). Proxies can be specified: see download.file.
...
# Support for the ftp:// scheme by the "internal" method was deprecated for R 4.1.1.
...
# Unix users wishing to write to one of the X11 selections may be able to do so via xclip (https://github.com/astrand/xclip) or xsel (http://www.vergenet.net/~conrad/software/xsel/), for example by pipe("xclip -i", "w") for the primary selection.
...
## Not run: % cat data2_
data2 <- c(450, 390, 467, 654,  30, 542, 334, 432, 421,
...
cat(x, file = (con <- file("books", "w", encoding = "UTF-8"))); close(con)
# read a 'Windows Unicode' file
# A <- read.table(con <- file("/home/admin/guile-rlang-project/guile-aicode/foo", encoding = "UCS-2LE")); close(con)
## End(Not run)
